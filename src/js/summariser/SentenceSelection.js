module.exports = function (topSentences, sentences) {
  const T = (Math.floor(sentences.length / 4) < 5) ? 5 : (Math.floor(sentences.length) / 4); // number of vertices to retain for post-processing, set between 5 and 20
  sentences = removeLineBreaks(sentences);
  // filter out one word sentences
  topSentences = topSentences.filter(item => countWords(sentences[item.vertex]) > 1);
  // Order by score
